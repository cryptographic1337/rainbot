...
const debug = require('debug')('rainbot:webclient');
const debugchat = require('debug')('rainbot:chat');
...
function WebClient(config) {
    ...
    debug('Setting up connection to ' + config.WEBSERVER);
...
WebClient.prototype.onMsg = function (msg) {
    debugchat('Msg: ', JSON.stringify(msg));
...
WebClient.prototype.onConnect = function (data) {
    debug('Web Server Connected. Received: ' + JSON.stringify(data));
